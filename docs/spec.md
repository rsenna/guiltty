...
- **Dev UX inspiration:** [ratatui](https://github.com/ratatui/ratatui) — its `Terminal<Backend>` + `Frame` + `draw(|frame| { ... })` immediate-mode render loop, composable `Widget` trait, and workspace split (`ratatui-core` / `ratatui-widgets` / backend crates) are adopted as the shape of guiltty's top-level API, given how well-known this UX is to Rust TUI developers
...
let mut canvas = terminal.canvas(800, 600)?;
canvas.set_zoom(1.5);
...
loop {
    terminal.draw(|frame| {
        ...
        frame.render_sprite(&sprite, Point::new(60, 50));
