...
- **Manual/visual verification**: example binaries in `examples/` are run by hand in a real kitty-protocol-compatible terminal as the acceptance check for actual rendering — this is not automated in v0, and is documented as a manual checklist rather than a CI gate. See [docs/spec-wezterm-e2e.md](spec-wezterm-e2e.md) for a planned automated tier (protocol-acceptance testing against a real terminal, headless, via WezTerm as a black-box dev-dependency) that narrows but does not eliminate this manual step.
